{{--
  Logo situs: favicon, header, pratinjau admin.
  @var string $previewUrl
  @var string $persistedLogoUrl
  @var bool $showUrlField
--}}
@php
    $previewUrl = $previewUrl ?? '';
    $persistedLogoUrl = $persistedLogoUrl ?? '';
    $showUrlField = $showUrlField ?? true;
    $logoPreviewSrc = \App\Support\PublicCmsUrl::imagePreviewSrc($previewUrl);
@endphp

<div class="space-y-4 rounded-lg border border-white/10 bg-church-surface/30 p-4">
    <div class="flex items-center gap-2">
        <span class="flex size-8 shrink-0 items-center justify-center rounded-md bg-amber-500/20 text-amber-300" aria-hidden="true">
        ...
        <span class="text-sm font-semibold text-church-fg">Logo</span>
    </div>

    <div>
        <input type="file" name="site_logo_file" accept="image/png,image/jpeg,image/webp,image/svg+xml,image/*" class="block w-full cursor-pointer rounded-lg border border-white/20 bg-church-surface px-3 py-2 text-sm text-slate-200 file:mr-4 file:rounded-md file:border-0 file:bg-slate-600 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-white ">
    </div>
 ...
 <button type="submit" name="site_logo_delete" value="1" class="public-btn-hover inline-flex items-center gap-2 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm font-medium text-slate-200 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400/50" onclick="return confirm('Hapus logo situs?');">
    ...
    <input type="hidden" name="site_logo_url_previous" value="{{ $persistedLogoUrl }}">
</div>

@if ($showUrlField)
    <div class="mt-4">
        <x-admin-field-label class="text-sm text-slate-300">URL logo — opsional</x-admin-field-label>
        <input name="site_logo_url" value="{{ $previewUrl }}" class="mt-1 w-full rounded-md border border-white/15 bg-church-surface px-3 py-2 text-sm text-church-fg">
    </div>
@endif
