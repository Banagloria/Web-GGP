...
@section('title', 'Pendaftaran Pernikahan')

@section('content')
    <div class="w-full min-w-0 max-w-full">
    <h1 class="mb-4 text-xl font-bold text-church-fg sm:text-2xl">Pendaftaran Pernikahan</h1>
    <div class="mb-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div class="flex min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            @include('admin.partials.btn', [
                'href' => route('dashboard.pendaftaran-pernikahan.create'),
                'variant' => 'primary',
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="bg-church-navy-mid text-white text-left">
                    ...
                    <th class="px-3 py-3 font-semibold">Mempelai pria</th>
                    <th class="px-3 py-3 font-semibold">Mempelai wanita</th>
            ...
            <tbody>
                @forelse ($items as $row)
                    <tr class="{{ $loop->even ? 'bg-admin-surface-zebra' : 'bg-church-card' }} border-t border-white/10">
                        ...
                        <td class="px-3 py-2 font-medium">{{ $row->groom_name }}</td>
                        <td class="px-3 py-2 font-medium">{{ $row->bride_name }}</td>
                        <td class="px-3 py-2">{{ $row->wedding_date?->timezone(config('app.timezone'))->format('d/m/Y') }}</td>
                        <td class="px-3 py-2">
                        ...
                        <td class="px-3 py-2">
                            @include('admin.partials.table-actions', [
                                'showUrl' => route('dashboard.pendaftaran-pernikahan.show', $row),
                                'editUrl' => route('dashboard.pendaftaran-pernikahan.edit', $row),
                                'deleteUrl' => route('dashboard.pendaftaran-pernikahan.destroy', $row),
                                'deleteMessage' => 'Data pernikahan ini akan dihapus permanen.',
                            ])
                        </td>
                    </tr>
                @empty
