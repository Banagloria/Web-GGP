...
@foreach ($sections as $si => $section)
    <div class="admin-panel space-y-4 rounded-lg border border-church-gold/20 bg-church-surface/40 p-4 sm:p-6">
        <h2 class="text-sm font-semibold text-church-gold">Bagian: {{ $section['title'] ?? 'Bagian '.($si + 1) }}</h2>
        ...
        <div class="grid gap-3 sm:grid-cols-2 sm:items-end">
            <div>
                ...
                <input name="sections[{{ $si }}][title]" value="{{ old('sections.'.$si.'.title', $section['title'] ?? '') }}" class="mt-1 w-full rounded-md border border-white/15 bg-church-surface text-sm text-church-fg">
            </div>
            @include('admin.partials.fa-icon-input', [
